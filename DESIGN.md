...
4. **Objects.** Models and preview plates float on a 7.5s cycle over a breathing bloom, drift with a small scroll parallax, and lean in on hover. Drag turns a model by hand; the orbit is clamped between 55deg and 95deg and pan, zoom and tap-to-recentre are off, so nothing can be pushed out of frame. On touch, a vertical swipe still scrolls the page.
